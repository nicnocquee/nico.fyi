...
date: '2024-10-11'
summary: For those who self host their Next.js app
tags: ['dev', 'nextjs']
authors: ['dennypradipta']
---

<div class="rounded-md border bg-green-100 px-4 py-2 dark:bg-slate-900">
...
Before we jump into handling long processes, let’s make sure we’ve got the basics covered. First off, you’ll need a Next.js app (but you probably already have that, right?) that you deploy to your own server. **Note that the guide I'm sharing here doesn't work with Vercel.**
...
As mentioned at the beginning, this technique is not possible to run in Vercel. In our project, we deploy both the Next.js app and the workers to our own server using [Docker](https://www.docker.com) and [dokku](https://dokku.com). To run the queues in production, we use the following steps:

...
- Replace `<service_uuid>` with the UUID of the pgbouncer service as seen in the URL in Coolify dashboard. `pgbouncer-<service_uuid>` is the essentially container name of the PgBouncer service assigned by Coolify. You can actually find the full container name by clicking the "Edit Compose File" in the PgBouncer+Postgres application page in your Coolify dashboard, then click "Show Deployable Compose". You will see the name of the container in `container_name` or `COOLIFY_CONTAINER_NAME`.

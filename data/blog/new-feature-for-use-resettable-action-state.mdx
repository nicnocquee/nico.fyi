...
In the example below, I'm using [the React reCAPTCHA v3 library](https://github.com/wojtekmaj/react-recaptcha-v3#readme) to acquire the token. This library has a `useGoogleReCaptcha` hook that returns a function to acquire the token, `executeRecaptcha`. Then I use the `beforeAction` function to acquire the token and modify the payload.
...
Additionally, the library is now 100% covered by tests as shown [here](https://github.com/nicnocquee/use-resettable-action-state/actions/runs/17187991759/job/48759677433#step:5:85)! 🥳
